...
impl SandboxPolicy {
        ...
        for component in path.components() {
            match component {
                std::path::Component::ParentDir => {
                    if !resolved.pop() {
                        return Err(anyhow!("Path traversal attempt: too many parent directory references"));
                    }
                }
    ...
    fn is_dangerous_command(&self, command: &str) -> bool {
        let dangerous_commands = [
            "rm", "del", "format", "mkfs", "dd", "fdisk", "parted",
            "sudo", "su", "chmod", "chown", "passwd", "userdel", "useradd",
            "systemctl", "service", "reboot", "shutdown", "halt", "poweroff",
            "iptables", "ufw", "firewall", "netsh", "route",
